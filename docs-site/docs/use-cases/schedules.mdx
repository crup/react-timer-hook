...
Use schedules when a timer needs side effects on a cadence. The timer lifecycle and schedule cadence are separate: `updateIntervalMs` updates UI, `everyMs` runs the callback.
...
For live demos, see [polling schedule](/recipes/intermediate/polling-schedule), [poll and cancel](/recipes/intermediate/poll-and-cancel), and [backend event stop](/recipes/intermediate/backend-event-stop).
