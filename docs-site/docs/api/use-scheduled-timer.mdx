...
`onError` comes from `UseTimerOptions` and receives sync throws or async rejections from `onEnd`. It is also the fallback for schedule callback errors when a schedule does not define its own `onError`.
